...
We also tested a multi-prompt SAM~2 fusion variant on \texttt{wild\_video2}; the result was informative but did not materially improve temporal coverage beyond the single-prompt SAM~2 result.
The project therefore provides four complementary outcomes: a reproducible classical baseline, a controlled backend replacement experiment, a targeted failure-to-improvement case study that isolates the role of mask quality, and a small ablation showing that simple prompt ensembling is not sufficient to further improve this sequence.
...
Finally, we test whether a slightly more tracking-aware variant that fuses several SAM~2 propagation runs can further stabilize the difficult case.
...
Our current Part~3 has two sub-branches.
Part~3A uses a single prompt-guided SAM~2 propagation run to produce refined masks for \texttt{wild\_video2}, then reruns ProPainter with the improved masks.
Part~3B is an ablation on temporal prompting: instead of a single prompt frame, we run SAM~2 from multiple non-empty seed frames and merge the propagated masks with either union or majority voting.
This second branch is important because it tests a concrete tracking-inspired hypothesis: perhaps the remaining failure is caused by over-reliance on one prompt frame rather than by the propagation model itself.
In parallel, we prepared a keyframe-level diffusion restoration branch for later comparison, but the current report focuses on the mask-side results because they are the branch that already produced complete and verifiable outputs.
...
For the diffusion branch, we prepared a keyframe-editing workspace on frames $75$, $113$, $151$, and $189$, but full SDXL-based results were not completed in time because the model weights were not yet mirrored into the offline server environment.
...
\begin{table}[t]
  \centering
  \caption{Ablation on \texttt{wild\_video2}: single-prompt vs.\ multi-prompt SAM~2 masks.}
  \small
  \begin{tabular}{lccc}
    \toprule
    Setting & Non-empty & Coverage & Mean area \\
    \midrule
    Part 1 mask & 104 & 0.547 & 0.0166 \\
    SAM~2 single & 122 & 0.642 & 0.0115 \\
    SAM~2 union & 122 & 0.642 & 0.0116 \\
    SAM~2 majority & 122 & 0.642 & 0.0115 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  \centering
  \IfFileExists{assets/wild_video2_part1_vs_sam2_multi_majority_part2.png}{
    \includegraphics[width=\linewidth]{assets/wild_video2_part1_vs_sam2_multi_majority_part2.png}
  }{
    \fbox{\rule{0pt}{1.2in}\rule{0.95\linewidth}{0pt}}
  }
  \caption{Multi-prompt SAM~2 majority-vote ablation on \texttt{wild\_video2}. The result remains clean, but it is visually very similar to the single-prompt SAM~2 result, matching the nearly unchanged temporal-coverage statistics in Table~4.}
\end{figure}
...
Fourth, our diffusion-based restoration branch is not yet complete.
We already prepared a standardized keyframe-editing workspace for \texttt{wild\_video2}, but the SDXL-based AttentiveEraser experiment requires large model weights and an accessible model mirror, so this report only includes the completed mask-side branch.
...
The most natural remaining extensions are to add either more mask-focused evaluation on an annotated subset, a synthetic benchmark for restoration metrics, or a completed diffusion-based keyframe restoration branch once the model deployment constraints are resolved.
