...
For object localization and instance segmentation, representative lines of work include early region-based detectors such as R-CNN~\cite{girshick2014rcnn}, later region-refinement models such as Mask R-CNN~\cite{he2017mask}, one-stage detectors such as YOLO~\cite{redmon2016yolo}, and transformer-based detectors such as DETR~\cite{carion2020detr}.
...
For restoration, earlier video inpainting systems such as Flow-Edge Guided Video Completion (FGVC)~\cite{gao2020fgvc} and flow-guided end-to-end video inpainting (E2FGVI)~\cite{li2022e2fgvi} already showed the importance of explicit temporal propagation and motion-aware completion.
ProPainter~\cite{zhou2023propainter} is a strong recent video inpainting model that combines propagation and transformer components to further improve temporal consistency and texture completion.
...
For our lightweight diffusion baseline, image-level generative priors based on latent diffusion~\cite{rombach2022ldm} and controllable adapters such as ControlNet~\cite{zhang2023controlnet} provide the most relevant conceptual background, even though our final recommendation remains video-level ProPainter rather than diffusion-first replacement.
For evaluation context, the DAVIS benchmark remains a standard reference for video object segmentation experiments~\cite{perazzi2016davis}, even though raw DAVIS masks do not by themselves provide clean-background ground truth for direct restoration-quality evaluation.

\section{Method}
\subsection{Part 1: Classical Baseline}
Our baseline follows a modular classical pipeline.
We first use YOLOv8-seg, a practical detector-segmenter in the YOLO family~\cite{redmon2016yolo}, to generate category-aware instance masks for likely dynamic objects such as people, bicycles, and vehicles.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{0.24\linewidth}p{0.16\linewidth}p{0.11\linewidth}p{0.35\linewidth}@{}}
    \toprule
    Sequence & Source & Frames & Role \\
    \midrule
    \texttt{bmx-trees} & Course & 40 & Annotated benchmark \\
    \texttt{tennis} & Course & 50 & Annotated benchmark \\
    \texttt{wild\_video1} & Own video & 190 & \shortstack[l]{Real-scene\\success case} \\
    \texttt{wild\_video2} & Own video & 190 & \shortstack[l]{Failure-to-\\improvement case} \\
    \bottomrule
  \end{tabular}
  \caption{Summary of the sequences used in the current project. The annotated course videos support mask evaluation, while the two wild videos provide real-scene qualitative analysis.}
...
\subsection{Metrics}
For mask quality, we report dataset-level averages of region similarity and recall-style coverage, denoted by $J_M$ and $J_R$, together with mean precision for additional interpretation.
If $M_t$ is the predicted mask and $G_t$ is the ground-truth mask on frame $t$, then the per-frame region similarity is the standard Jaccard index
\begin{equation}
  J(M_t, G_t) = \frac{|M_t \cap G_t|}{|M_t \cup G_t|}.
\end{equation}
We then report the sequence average
\begin{equation}
  J_M = \frac{1}{T} \sum_{t=1}^{T} J(M_t, G_t).
\end{equation}
To measure whether the predicted mask sufficiently covers the ground-truth foreground, we define the per-frame recall-style coverage
\begin{equation}
  R(M_t, G_t) = \frac{|M_t \cap G_t|}{|G_t|},
\end{equation}
and average it over the sequence as
\begin{equation}
  J_R = \frac{1}{T} \sum_{t=1}^{T} R(M_t, G_t).
\end{equation}
...
We additionally report mean precision to capture the complementary tendency of the predicted masks to over-expand into background regions.
For restoration quality, direct pixel-wise evaluation is only meaningful when a clean target frame exists.
...
\begin{figure}[t]
  \centering
  \IfFileExists{assets/generated/benchmark_mask_metrics.pdf}{
    \includegraphics[width=\linewidth]{assets/generated/benchmark_mask_metrics.pdf}
  }{
...
At the current project stage, the restoration story is best supported by controlled qualitative comparison.
...
\begin{figure*}[t]
  \centering
  \IfFileExists{assets/generated/wild_video2_ablation_metrics.pdf}{
    \includegraphics[width=\textwidth]{assets/generated/wild_video2_ablation_metrics.pdf}
  }{
...
\begin{figure*}[t]
  \centering
  \IfFileExists{assets/generated/course_sequence_comparisons.pdf}{
    \includegraphics[width=\textwidth]{assets/generated/course_sequence_comparisons.pdf}
  }{
    \fbox{\rule{0pt}{2.8in}\rule{0.98\textwidth}{0pt}}
  }
  \caption{Controlled qualitative comparisons on the two annotated course sequences. The content is unchanged relative to the earlier PNG grids, but the figure is reformatted with larger external labels and fewer sampled rows so that the restoration difference between Part~1 and ProPainter is easier to read.}
  \label{fig:course-qualitative}
\end{figure*}

\begin{figure*}[t]
  \centering
  \IfFileExists{assets/generated/wild_sequence_comparisons.pdf}{
    \includegraphics[width=\textwidth]{assets/generated/wild_sequence_comparisons.pdf}
  }{
    \fbox{\rule{0pt}{2.8in}\rule{0.98\textwidth}{0pt}}
  }
  \caption{Real-scene qualitative comparisons. The top block shows the success case on \texttt{wild\_video1}; the bottom block shows the failure-to-improvement case on \texttt{wild\_video2}, where prompt-guided SAM~2 refinement yields better masks and a cleaner final restoration than the original failure case.}
  \label{fig:wild-qualitative}
\end{figure*}
...
Together, these experiments make the report stronger than a single baseline-only implementation.
...
Within the current project scope, the strongest evidence supports a pipeline that combines better temporal masks with a stronger video inpainting backend.
